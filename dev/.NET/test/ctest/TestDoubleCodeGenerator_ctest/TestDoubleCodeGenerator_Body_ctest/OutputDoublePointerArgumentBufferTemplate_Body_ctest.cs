...
{
	internal class OutputDoublePointerArgumentBufferTemplate_Body_ctest
	{
		[OneTimeSetUp]
		...
		public void Test1()
		{
			...
			var template = new OutputDoublePointerArgumentBufferTemplate()
			{
				Function = function,
				Target = argument
			};
			...
			string expect =
				...
				"\tfor (int index = 0; index < SampleFunction_Argument1_return_value_size[SampleFunction_called_count]; index++) {" + Environment.NewLine +
				"\t\t*Argument1[index] = &SampleFunction_Argument1_return_value[SampleFunction_called_count][index];" + Environment.NewLine +
				"\t}" + Environment.NewLine;
